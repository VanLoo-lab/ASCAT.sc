plotSunrise <- function(solution, localMinima = FALSE, plotClust=FALSE, N=10)
{
  tryCatch({
    ...
    axis(side = 2, at = seq(0.1, 1, 0.1), signif(as.numeric(rownames(errs)[(nrow(errs):1)[round(seq(0.1,
                                                                                                    1, 0.1) * nrow(errs))]]), 2))
    text(0.85, 0.1, cex = 0.9, as.expression(bquote(paste("max ",
                                                         phi[T], " hit"))), col = rgb(1, 1, 1))

    if(localMinima) return(bao)
